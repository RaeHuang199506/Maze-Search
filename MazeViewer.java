   ...
   */
   private static MazeFrame readMazeFile(String fileName) throws IOException {
      ...
      try (Scanner in = new Scanner(inFile)){
    	  ...
    	  return new MazeFrame(mazeData, readMazeCoord(in), readMazeCoord(in));
      }
   }
    ...
    * @param in values scanned from the specified file
    * @return the maze
    */
   private static boolean[][] readMazeData(Scanner in) {
   ...
   /**
    * Read the start and exit location of the maze
    * @param in values scanned from the specified file
    * @return the coordinate of the location
    */
   private static MazeCoord readMazeCoord(Scanner in) {
      String[] s = in.nextLine().split(" ");
      int row = Integer.parseInt(s[0]);
      int col = Integer.parseInt(s[1]);
      return new MazeCoord(row,col);
   }
}
